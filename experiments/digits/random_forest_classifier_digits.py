"""Experiment script for RandomForestClassifier on the Digits dataset."""
...
from src.models.random_forest_classifier_model import RandomForestClassifierModel
...
def run_rf_without_inference():
    """Run the Random Forest Classifier baseline (no inference or perturbations).
    ...
    report_data("=== RandomForestClassifier WITHOUT INFERENCE ===", mode=ReportMode.PRINT)
    base_params = {"n_estimators": 100, "random_state": 42}
    filtered_params = filter_sklearn_params(base_params, model_class)
...
def run_rf_with_inference():
    """Run RandomForestClassifier with data, parameter, and label inference.

    Loads the Digits dataset, applies full inference (data, parameter, label),
    trains a RandomForestClassifier model, evaluates it, and reports results.
    """
    model_class = RandomForestClassifierModel
    dataset_name = SklearnDatasetName.DIGITS
    name_output = generate_experiment_filename(model_class, dataset_name)

    report_data("=== RandomForestClassifier WITH INFERENCE (data + param + label) ===",
    ...
    filtered_params = filter_sklearn_params(perturbed_params, RandomForestClassifierModel)
    model = RandomForestClassifierModel(**filtered_params)
...
def run():
    """Runs the complete experiment suite for the RandomForestClassifier."""
    run_rf_without_inference()
    run_rf_with_inference()
