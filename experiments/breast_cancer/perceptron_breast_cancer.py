...
def run_perceptron_without_inference():
    ...
    dataset = DatasetFactory.create(DatasetSourceType.SKLEARN, name=SklearnDatasetName.BREAST_CANCER)
...
def run_perceptron_with_inference():
    ...
    report_data(metrics, mode=ReportMode.PRINT)
    report_data(param_log, mode=ReportMode.JSON, file_path='results/perceptron_param_log-breast_cancer.json')
