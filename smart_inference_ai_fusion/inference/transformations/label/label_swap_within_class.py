...
class LabelSwapWithinClass(LabelTransformation):
    ...
    def apply(self, y: ArrayLike) -> np.ndarray:
        """Swap pairs of labels within each superclass up to a target fraction.

        Args:
            y (ArrayLike): Input label vector (1-D). Will be converted to ``str`` internally.
                Shape: (n_samples,).
        ...
        Notes:
            - O número total de trocas é limitado por ``swap_within_class_fraction``.
            - Cada rótulo é considerado no formato ``"<superclass><delimiter><subclass>"``.
            Apenas rótulos com o mesmo ``superclass`` podem ser trocados entre si.
