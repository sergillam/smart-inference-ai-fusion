...
class InferencePipeline:
    ...
    def __init__(self, data_noise_config=None, label_noise_config=None, X_train=None, 
        ...
        if verification_config:
            logger.warning("verification_config parameter is deprecated. Use environment variables instead.")
    ...
    def _verify_parameters_multi_solver(self, step_name: str, params: Dict[str, Any], 
                                       original_params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if self.config.parallel_solvers and len(self._verifier_cache) > 1:
            ...
            results = self._verify_parallel(
                f"pipeline.parameters.{step_name}", 
                constraints, 
                params
            )
        else:
    ...
    def _verify_parallel(self, name: str, constraints: Dict[str, Any], 
        ...
        with ThreadPoolExecutor(max_workers=self.config.max_workers) as executor:
            ...
            for future in as_completed(future_to_solver):
                ...
                try:
                    result = future.result(timeout=self.config.timeout_per_constraint + 10)  # +10s buffer
                    results[solver_name] = result
    ...
    def _verify_with_solver(self, verifier, name: str, constraints: Dict[str, Any], 
                           input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        from smart_inference_ai_fusion.verification.core.error_handling import handle_verification_error, should_disable_solver
        ...
        try:
            ...
            error_result = handle_verification_error(e, solver_name, "pipeline_verification", error_context)
            ...
            if error_result.get("action") == "switch_solver":
                ...
                if new_solver_name and new_solver_name in self._verifier_cache:
                    ...
                    try:
                        return self._verify_with_solver(fallback_verifier, name, constraints, input_data)
                    except Exception as fallback_error:
                        logger.error(f"❌ Fallback also failed: {fallback_error}")
            
            return {
                "status": "ERROR",
                "message": error_result.get("message", f"{solver_name} verification failed: {str(e)}"),
                "execution_time": 0.0,
                "constraints_checked": [],
                "constraints_satisfied": [],
                "constraints_violated": [],
    ...
    def _compare_verification_results(self, results: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        comparison["agreement"]["common_status"] = list(unique_statuses)[0] if len(unique_statuses) == 1 else None
        ...
        comparison["status_summary"] = {
            solver: result.get("status", "ERROR")
            for solver, result in solver_results.items()
        }
        
        return comparison
    
    def _build_parameter_constraints(self, params: Dict[str, Any], 
                                   original_params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
