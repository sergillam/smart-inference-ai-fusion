"""Experiment script for MiniBatchKMeans on the Digits dataset."""
...
def run_minibatch_kmeans_without_inference():
    """Run the MiniBatchKMeans baseline (no inference or perturbations)."""
    ...
    report_data("=== MiniBatchKMeans WITHOUT INFERENCE ===", mode=ReportMode.PRINT)
    base_params = {"n_clusters": 10, "random_state": 42, "batch_size": 100}
    filtered_params = filter_sklearn_params(base_params, MiniBatchKMeansModel)
...
def run_minibatch_kmeans_with_inference():
    """Run the MiniBatchKMeans model with inference."""
    model_class = MiniBatchKMeansModel
    dataset_name = SklearnDatasetName.DIGITS
    name_output = generate_experiment_filename(model_class, dataset_name)

    report_data(
        "=== MiniBatchKMeans WITH INFERENCE (data + param + label) ===", mode=ReportMode.PRINT
    )
    ...
    filtered_params = filter_sklearn_params(perturbed_params, MiniBatchKMeansModel)
    model = MiniBatchKMeansModel(**filtered_params)
...
def run():
    ...
    run_minibatch_kmeans_without_inference()
    run_minibatch_kmeans_with_inference()
