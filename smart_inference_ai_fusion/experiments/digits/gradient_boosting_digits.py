"""Experiment script for GradientBoosting on the Digits dataset."""
...
def run_gbdt_without_inference():
    """Run the GradientBoosting baseline (no inference or perturbations).

    Loads the Digits dataset, trains a GradientBoosting model, evaluates it,
    ...
    report_data("=== GradientBoostingModel WITHOUT INFERENCE ===", mode=ReportMode.PRINT)
    base_params = {"var_smoothing": 1e-9}
    filtered_params = filter_sklearn_params(base_params, GradientBoostingModel)
...
def run_gbdt_with_inference():
    """Run GradientBoostingModel with data, parameter, and label inference (perturbations).

    Loads the Digits dataset, applies full inference (data, parameter, label),
    trains a GradientBoostingModel, evaluates it, and reports results via report_data.
    ...
    report_data(
        "=== GradientBoostingModel WITH INFERENCE (data + param + label) ===", mode=ReportMode.PRINT
    )
    ...
    filtered_params = filter_sklearn_params(perturbed_params, GradientBoostingModel)
    model = GradientBoostingModel(**filtered_params)
...
def run():
    """Runs the complete experiment suite for the GradientBoostingModel."""
    run_gbdt_without_inference()
    run_gbdt_with_inference()
