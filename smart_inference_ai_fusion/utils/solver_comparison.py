...
class SolverComparison:
    ...
    def add_verification_results(self, experiment_name: str, 
    ...
    def _analyze_performance(self, solver_results: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "time_ratio": execution_times[slowest_solver] / execution_times[fastest_solver] if execution_times[fastest_solver] > 0 else float('inf')
        }
    
    def generate_comparison_report(self, experiment_name: str = "multi_solver_comparison") -> ComparisonResult:
        ...
        performance_winner = max(performance_wins, key=performance_wins.get) if performance_wins else "N/A"
        reliability_winner = max(reliability_wins, key=reliability_wins.get) if reliability_wins else "N/A"
    ...
    def _save_text_report(self, result: ComparisonResult, base_filename: str):
        ...
        with open(txt_file, 'w', encoding='utf-8') as f:
            ...
            for solver_name, metrics in result.solver_metrics.items():
                ...
                f.write(f"  Tempo mín/máx: {metrics.min_execution_time:.4f}s / {metrics.max_execution_time:.4f}s\n")
            ...
            if "success_rate" in result.summary.get("solver_rankings", {}):
                f.write(f"Taxa de Sucesso: {' > '.join(result.summary['solver_rankings']['success_rate'])}\n")
            
            if "average_speed" in result.summary.get("solver_rankings", {}):
                f.write(f"Velocidade Média: {' > '.join(result.summary['solver_rankings']['average_speed'])}\n")
            
            if "reliability" in result.summary.get("solver_rankings", {}):
                f.write(f"Confiabilidade: {' > '.join(result.summary['solver_rankings']['reliability'])}\n")
    ...
    def print_summary(self):
        ...
        for solver_name, metrics in self.solver_metrics.items():
            logger.info(f"\n{solver_name}:")
            logger.info(f"  ✅ Successes: {metrics.successful_verifications}/{metrics.total_verifications} ({metrics.success_rate:.1%})")
...
class StandardizedSolverComparison:
    ...
    def add_legacy_result(self, legacy_result, solver_name: str, solver_version: str = "unknown"):
        ...
        standard_result = self.schema_manager.standardize_result(legacy_result, solver_name, solver_version)
    ...
    def _generate_detailed_analysis(self) -> Dict[str, Any]:
        ...
        for result in self.standard_results:
            ...
            analysis["performance_distribution"][solver_name].append(result.performance.total_execution_time)
    ...
    def export_standard_results(self, output_dir: str = "results/standard_comparison") -> Dict[str, str]:
...
def generate_comparison_report(experiment_name: str = "multi_solver_comparison") -> ComparisonResult:
