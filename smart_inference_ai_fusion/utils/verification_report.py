...
def _report_console_verification(details: Dict[str, Any]) -> None:
    ...
    report_data(f"🔍 {status_icon} FORMAL VERIFICATION - {info['verifier']}", mode=ReportMode.PRINT)
    ...
    if context["transformation_name"] != "pipeline_verification":
        report_data(
            f"   🔧 Transformation: {context['transformation_name']}", mode=ReportMode.PRINT
        )
    ...
    if constraints["total_checked"] > 0:
        ...
        if constraints["constraints_satisfied"]:
            ...
            report_data(f"   ✅ Satisfied: {satisfied_list}", mode=ReportMode.PRINT)

        # List violated constraints (if any)
        if constraints["constraints_violated"]:
            violated_list = ", ".join(constraints["constraints_violated"])
            report_data(f"   ❌ Violated: {violated_list}", mode=ReportMode.PRINT)

    # Mensagem adicional do solver
    if info["message"]:
        report_data(f"   💬 Z3: {info['message']}", mode=ReportMode.PRINT)


def _save_verification_results(
    details: Dict[str, Any], model_name: str, dataset_name: str, transformation_name: Optional[str]
) -> None:
    """Salva resultados detalhados da verificação em JSON."""
    # Gerar nome do arquivo
    suffix = (
        f"verification-{transformation_name}" if transformation_name else "verification-pipeline"
    )
    ...
    report_data(content=details, mode=ReportMode.JSON_RESULT, name_output=filename)
...
def _log_verification_details(details: Dict[str, Any]) -> None:
    ...
    report_data(content=log_entry, mode=ReportMode.JSON_LOG, name_output=log_name)


def enhance_experiment_results_with_verification(
    experiment_results: Dict[str, Any], verification_results: list[VerificationResult]
) -> Dict[str, Any]:
...
def report_solver_sat_unsat_details(solver_details: Dict[str, Any], constraint_name: str) -> None:
    ...
    if "satisfiable" in solver_details:
        ...
        report_data(f"   🧮 Z3 Result for '{constraint_name}': {sat_status}", mode=ReportMode.PRINT)

    # Reportar modelo (se SAT)
    if solver_details.get("model"):
        report_data(f"   📝 Z3 Model: {solver_details['model']}", mode=ReportMode.PRINT)

    # Reportar core insatisfazível (se UNSAT)
    if solver_details.get("unsat_core"):
        report_data(f"   🔍 Z3 Unsat Core: {solver_details['unsat_core']}", mode=ReportMode.PRINT)

    # Reportar estatísticas do solver
    if solver_details.get("statistics"):
        stats = solver_details["statistics"]
        report_data(f"   📊 Z3 Stats: {stats}", mode=ReportMode.PRINT)
