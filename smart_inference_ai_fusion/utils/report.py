...
from .types import ReportMode


def get_iso_timestamp() -> str:
    """Return the current UTC time as a filename-safe ISO string.

    The string is formatted as ``YYYY-MM-DDTHH-MM-SSZ`` and is safe to use
    in filenames.

    Returns:
        str: UTC timestamp formatted as a filename-safe ISO string.
    """
...
def generate_experiment_filename(
    model_class: Any, dataset_name: Any, suffix: Optional[str] = None, max_len: int = 100
) -> str:
...
def report_data(content: dict | str, mode: ReportMode, name_output: str = None) -> None:
