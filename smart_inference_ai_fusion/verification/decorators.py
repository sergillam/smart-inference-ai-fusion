        ...
        def wrapper(*args, **kwargs):
            ...
            if enabled and verification_manager.enabled:
                ...
                if fail_on_error and verification_result.status == VerificationStatus.FAILED:
                    raise RuntimeError(f"Verification failed for {func.__name__}: {verification_result.message}")
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(self, *args, **kwargs):
            ...
            if enabled and verification_manager.enabled:
                ...
                logger.info(f"Model verification for {func.__name__}: {verification_result.status.value}")
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            if enabled and verification_manager.enabled:
                ...
                logger.debug(f"Pipeline step verification for {step_name}: {verification_result.status.value}")
