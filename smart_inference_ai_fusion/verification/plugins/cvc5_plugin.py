...
class CVC5Verifier(FormalVerifier):
    ...
    def _verify_bounds_constraint(
        ...
        try:
            ...
            constraints_added = 0
            if min_val != float("-inf") and not (
                isinstance(min_val, str) and min_val in ["True", "False"]
            ):
                try:
                    min_constraint = self.solver.mkTerm(
                        Kind.GEQ, x, self.solver.mkReal(str(float(min_val)))
                    )
            ...
            if max_val != float("inf") and not (
                isinstance(max_val, str) and max_val in ["True", "False"]
            ):
                try:
                    max_constraint = self.solver.mkTerm(
                        Kind.LEQ, x, self.solver.mkReal(str(float(max_val)))
                    )
    ...
    def _verify_parameter_drift_constraint(
        ...
        try:
            ...
            threshold = (
                ...
                else float(constraint_value) if isinstance(constraint_value, (int, float)) else 0.1
            )
