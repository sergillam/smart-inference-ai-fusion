...
class PerformanceMetrics:
    ...
    def completion_rate(self) -> float:
        ...
        if self.constraint_count == 0:
            return 0.0
        completed = (
            self.constraints_satisfied + self.constraints_violated + self.constraints_unknown
        )
...
class StandardVerificationResult:
    ...
    def from_legacy_result(
        cls, legacy_result, solver_name: str, solver_version: str = "unknown"
        ...
        overall_status = status_mapping.get(
            getattr(legacy_result, "status", "UNKNOWN").upper(), StandardStatus.UNKNOWN
        )
        ...
        for constraint_name in constraints_satisfied:
            constraint_results.append(
                ConstraintResult(
                    ...
                    execution_time=(
                        execution_time / len(constraints_checked) if constraints_checked else 0.0
                    ),
                )
            )
    ...
    def compare_with(self, other: "StandardVerificationResult") -> Dict[str, Any]:
        ...
        comparison = {
            ...
            "execution_time_comparison": {
                ...
                "faster_solver": (
                    self.solver_metadata.solver_name
                    if self.performance.total_execution_time
                    < other.performance.total_execution_time
                    else other.solver_metadata.solver_name
                ),
                "speedup": abs(
                    self.performance.total_execution_time - other.performance.total_execution_time
                )
                / max(
                    self.performance.total_execution_time, other.performance.total_execution_time
                ),
            ...
            "accuracy_comparison": {
                ...
                "more_accurate": (
                    self.solver_metadata.solver_name
                    if self.performance.success_rate > other.performance.success_rate
                    else other.solver_metadata.solver_name
                ),
            },
    ...
    def _calculate_constraint_agreement(
        self, other: "StandardVerificationResult"
    ) -> Dict[str, Any]:
...
class ResultSchemaManager:
    ...
    def standardize_result(
        legacy_result, solver_name: str, solver_version: str = "unknown"
    ) -> StandardVerificationResult:
        """Padroniza resultado legado."""
        return StandardVerificationResult.from_legacy_result(
    ...
    def compare_results(results: List[StandardVerificationResult]) -> Dict[str, Any]:
        ...
        comparison["ranking"] = {
            ...
            "by_accuracy": sorted(
                [(r.solver_metadata.solver_name, r.performance.success_rate) for r in results],
                key=lambda x: x[1],
                reverse=True,
            ),
            "by_speed": sorted(
                [
                    (r.solver_metadata.solver_name, r.performance.total_execution_time)
                    for r in results
                ],
                key=lambda x: x[1],
            ),
            "by_completion": sorted(
                [(r.solver_metadata.solver_name, r.performance.completion_rate) for r in results],
                key=lambda x: x[1],
                reverse=True,
            ),
        }
        ...
        comparison["summary"] = {
            ...
            "fastest": min(
                results, key=lambda r: r.performance.total_execution_time
            ).solver_metadata.solver_name,
            "most_accurate": max(
                results, key=lambda r: r.performance.success_rate
            ).solver_metadata.solver_name,
            "most_complete": max(
                results, key=lambda r: r.performance.completion_rate
            ).solver_metadata.solver_name,
            "average_execution_time": sum(r.performance.total_execution_time for r in results)
            / len(results),
